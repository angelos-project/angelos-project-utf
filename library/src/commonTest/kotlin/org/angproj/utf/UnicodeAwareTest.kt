...
class UnicodeAwareTest : UnicodeAware {
    ...
    fun testOctetReadBlk() {
        listOf(Policy.passthrough, Policy.escape, Policy.security).forEach { policy ->
            ...
            val outStrm1 = ByteArray(strm1.size)
            while (inPos < strm1.size) {
                val cp = readGlyphByPolicyBlk(strm1.size-inPos, policy) { strm1[inPos++] }
                writeGlyphByPolicyBlk(cp, outStrm1.size-outPos, policy) { outStrm1[outPos++] = it }
            }
            assertContentEquals(strm1, outStrm1)
            ...
            val outStrm2 = ByteArray(strm2.size)
            while (inPos < strm2.size) {
                val cp = readGlyphByPolicyBlk(strm2.size-inPos, policy) { strm2[inPos++] }
                writeGlyphByPolicyBlk(cp, outStrm2.size-outPos, policy) { outStrm2[outPos++] = it }
            }
            assertContentEquals(strm2, outStrm2)
            ...
            val outStrm3 = ByteArray(strm3.size)
            while (inPos < strm3.size) {
                val cp = readGlyphByPolicyBlk(strm3.size-inPos, policy) { strm3[inPos++] }
                writeGlyphByPolicyBlk(cp, outStrm3.size-outPos, policy) { outStrm3[outPos++] = it }
            }
            assertContentEquals(strm3, outStrm3)
            ...
            val outStrm4 = ByteArray(strm4.size)
            while (inPos < strm4.size) {
                val cp = readGlyphByPolicyBlk(strm4.size-inPos, policy) { strm4[inPos++] }
                writeGlyphByPolicyBlk(cp, outStrm4.size-outPos, policy) { outStrm4[outPos++] = it }
            }
            assertContentEquals(strm4, outStrm4)
        }
    }
